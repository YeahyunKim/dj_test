# 송금 서비스 코딩테스트 과제 (Spring Boot + JPA)

계좌 등록/삭제, 입금, 출금(일 한도), 이체(수수료/일 한도), 거래내역 조회 기능을 제공하는 간단한 송금 서비스 API입니다.  
요구사항에 맞춰 **Java 17**, **Spring Boot**, **JPA(Hibernate)** 기반으로 구현하며, **Docker Compose**로 실행 환경을 제공합니다.
...
- (DB) MySQL 또는 PostgreSQL (Docker Compose 기반)
...
### 3.4 Remove Volumes (DB 초기화)

```bash
docker compose down -v
```

### 3.5 Check Containers / Logs

```bash
# 컨테이너 상태 확인
docker compose ps

# 앱 로그 확인 (서비스명이 app인 경우)
docker compose logs -f app

# DB 로그 확인 (서비스명이 db인 경우)
...
| 입금 | POST | `/api/accounts/{accountNumber}/deposit` | 금액 입금 |
| 출금 | POST | `/api/accounts/{accountNumber}/withdraw` | 금액 출금(한도/잔액 체크) |
...
![ERD](https://github.com/user-attachments/assets/a0f19164-72dc-4c7f-8647-8468ca6f635e)
...
- 동시성 고려(우대사항)
  - 잔액/한도 업데이트 구간에서 Lock(낙관/비관) 적용 또는 트랜잭션 경계 명확화로 정합성을 유지합니다.
